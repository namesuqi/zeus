...
class ExecutorLive(ExecutorBase):
    def deploy_agent(self):
        ...
        for ip in PEER_IP_LIST:
            nodes.append(RemoteNodeSdk(ip, PEER_USER, PEER_PASSWORD))
        for node in nodes:
            commands = list()
            print "------------------------------------"
            print "deploy_agent for {0}".format(node.ip)
            commands.append(
                'ssh -o StrictHostKeyChecking=no {1}@{2} "rm -rf ~/zeus_agent; mkdir ~/zeus_agent"'
                    .format(node.password, node.user, node.ip))
            commands.append(
                'scp -r {3}/lib/zeus_agent/live* {1}@{2}:~/zeus_agent'
                    .format(node.password, node.user, node.ip, root_path))
            commands.append(
                'ssh {1}@{2} "chmod -R 755 ~/zeus_agent; cd ~/zeus_agent; ./live_restart.sh > /dev/null 2>&1"'
                    .format(node.password, node.user, node.ip))
    ...
    def deploy_sdk(self):
        ...
        for node in nodes:
            ...
            commands.append(
                'ssh -o StrictHostKeyChecking=no {1}@{2} "rm -rf ~/live; mkdir ~/live"'
                    .format(node.password, node.user, node.ip))
            commands.append(
                'scp -r {3}/misc/bin/live/ubuntu/* {1}@{2}:~/live'
                    .format(node.password, node.user, node.ip, root_path))
            commands.append(
                'ssh {1}@{2} "chmod -R 755 ~/live"'
                    .format(node.password, node.user, node.ip))
            for command in commands:
                p = subprocess.Popen(command, shell=True)
                p.wait()  # wait process finish to avoid parallel execution
