            ...
            for i in range(288):
                if (timestamp + 300*1000*(i+1)) > long(timesp) >= (timestamp + 300*1000*i):
                    ...
                    if playtype.startswith('live'):
                        typelist[i][tmpusername]['live']['totaldown'] = typelist[i][tmpusername]['live'].setdefault('totaldown', 0) + int(cdndown) + int(p2pdown)
                    ...
                    resultlist[i][tmpusername][playtype]['cdndown'] = resultlist[i][tmpusername][playtype].setdefault('cdndown', 0) + long(cdndown)
                    resultlist[i][tmpusername][playtype]['p2pdown'] = resultlist[i][tmpusername][playtype].setdefault('p2pdown', 0) + long(p2pdown)
