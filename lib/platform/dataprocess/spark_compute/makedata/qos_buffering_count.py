...
class QosBufferingCount(object):

    def make_data(self, hour=''):
        data_format = '%s\x1f%s\x1f%s\x1f%s\x1f%s\x1f%s\x1f%s\x1f%s\x1f%s\x1f%s\x1f%s\x1f%s\n'
        with open(os.path.abspath(os.path.dirname(__file__)) + '/qos_buffering_count.txt', 'w') as writer:
            if hour == '':
                for hour in range(24):
                    for minute in range(60):
                        for i in range(random.randint(0, 2)):
                            topic = 'topic=' + 'qos_buffering_count'
                            ...
                            buffering_count = 'buffering_count=' + random.randint(1, 20)
                            ...
                            writer.write(data_format % (topic, id, timestamp, peer_id, play_type, url, vvid, buffering_count
                                                    , public_ip, input_time, sdk_agent_name, sdk_agent_version))
            else:
                for minute in range(60):
                    for i in range(random.randint(0, 2)):
                        topic = 'topic=' + 'qos_buffering_count'
                        ...
                        url = 'url=' + 'http://testcloudtropy.com'
                        vvid = 'vvid=' + CreateHexString.create_by_length(32)
                        buffering_count = 'buffering_count=' + str(random.randint(1, 20))
                        public_ip = 'public_ip=' + '10.5.100.1'
                        input_time = 'input_time=' + str(long(timestamp.split('=')[1]) + 300)
                        sdk_agent_name = 'sdk_agent_name=' + 'YunshangSDk'
                        sdk_agent_version = 'sdk_agent_version=' + test_sdk_agent_version[
