...
class TotalFlowHour(object):
    ...
    def compute(self, partition_time=''):
        ...
        for line in download_origin_lines:
            ...
            if partition_time == '':
                day_start = TimestampConversion.get_timestamp(test_day + now_hour)
                day_end = TimestampConversion.get_timestamp(test_day + str(int(now_hour)+1))
                ...
                day_start = TimestampConversion.get_timestamp(partition_time)
                day_end = TimestampConversion.get_timestamp(str(int(partition_time)+1))
                if long(day_start) <= long(timestamp) < long(day_end):
                    if day_start not in download_count:
                        download_count[day_start] = dict()
                    if prefix not in download_count[day_start]:
        ...
        for timestamp in download_count.keys():
            ...
            for user in download_count[timestamp].keys():
                ...
                for play_type in download_count[timestamp][user].keys():
                    ...
                    if 'all' not in all_download_count[timestamp]['all']:
                        all_download_count[timestamp]['all']['all'] = dict()
                        all_download_count[timestamp]['all']['all']['cdn_download'] = 0
                        all_download_count[timestamp]['all']['all']['p2p_download'] = 0
                        all_download_count[timestamp]['all']['all']['total_download'] = 0
                ...
                for username in download_count[timestamp].keys():
                    for play_type in download_count[timestamp][username].keys():
                        download_expect_file.write('%s,%s,%s,download:cdn,%d\n' % (
                            username, timestamp, play_type,
                            download_count[timestamp][username][play_type]['cdn_download']))
                        download_expect_file.write('%s,%s,%s,download:p2p,%d\n' % (
                            username, timestamp, play_type,
                            download_count[timestamp][username][play_type]['p2p_download']))
                        download_expect_file.write('%s,%s,%s,download:total,%d\n' % (
                            username, timestamp, play_type,
                            download_count[timestamp][username][play_type]['total_download']))
...
if __name__ == '__main__':
    eg = TotalFlowHour()
    eg.compute(partition_time='2016101422')
