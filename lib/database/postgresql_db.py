...
import string
import time
from lib.interface.boss.environment_constant import BOSS_POSTGRESQL_DATABASE, \
    BOSS_POSTGRESQL_USER, BOSS_POSTGRESQL_PASSWORD, BOSS_POSTGRESQL_HOST, BOSS_POSTGRESQL_PORT


class PostgresqlDB(object):
    def __init__(self, database=BOSS_POSTGRESQL_DATABASE, user=BOSS_POSTGRESQL_USER, password=BOSS_POSTGRESQL_PASSWORD,
                 host=BOSS_POSTGRESQL_HOST, port=BOSS_POSTGRESQL_PORT):
        start_time = time.time()
        print "Connect to PostgreSQL database by {0}, it should be only call one time!!!".format(user)
    ...
    def __del__(self):
        self._conn.close()
        print "Disconnect to PostgreSQL"
    ...
    def __has_alias__(self):
        ...
        if string.rfind(columns, " * ") == -1:
            ...
            # 获取数据表的所有列名
            self._cur.execute("select column_name from information_schema.columns where table_schema='public' and table_name = '{0}'".format(table_name))
