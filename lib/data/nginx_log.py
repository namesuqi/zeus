...
class NginxLogParser(object):
    ...
    def __init__(self, nginx_log):
        ...
        for line in fileinput.input(nginx_log):
            ...
            if matchs != None:
                ...
                # 统计HTTP状态码的数量
                self.get_response_status_count(status)
                # 在这里补充其他任何需要的分析代码
            else:
                raise Exception
    ...
    # 统计HTTP状态码的数量
    def get_response_status_count(self, status):
        if status in self.all_status_dict:
            self.all_status_dict[status] += 1
        else:
            self.all_status_dict[status] = 1
...
if __name__ == "__main__":
    parser = NginxLogParser("e:/log/access.log")
    print "origin:", parser.all_status_dict
    #根据值进行排序（倒序）
    print "sorted:", sorted(parser.all_status_dict.items(), key=lambda d:d[1], reverse=True)
