...
---- Reserved and Type
    ...
    local function yspuff_detector(buffer, pinfo, tree)
        ...
        local type = bit:_and(buffer(0, 1):uint(), 0x0F)
        if type == C_PUFF_SESSION_REQ then
            if L ~= 293 + 2 then return false end
        elseif type == C_PUFF_SESSION_RSP then
            if L ~= 3 + 2 then return false end
        elseif type == C_PUFF_SESSION_DAT then
            if L ~= 874 + 2 and L ~= 874+8+2 then return false end
        elseif type == C_PUFF_SESSION_HIB then
            if L ~= 34 + 2 then return false end
        elseif type == C_PUFF_SESSION_FIN then
            if L ~= 35 + 2 then return false end
        end
        return true
    end
---------------------------------------------------------------------------------------------------
...
---------------------------------------------------------------------------------------------------
    ...
    local function ysrrpc_dissector(buffer, pinfo, tree)
        ...
        rrpc_tree:add(f_pf_rrpc_checksum, buffer(offset, 2), buffer(offset, 2):uint())
        offset = offset + 2
...
---------------------------------------------------------------------------------------------------
    ...
    local function yscccp_dissector(buffer, pinfo, tree)
        ...
        cccp_tree:add(f_pf_cccp_ui16_checksum, buffer(offset, 2), buffer(offset, 2):uint())
        offset = offset + 2
