...
---------------------------------------------------------------------------------------------------
    ...
    C_SUPP_OPT_END       = 0
    C_SUPP_OPT_SEQ_NACK  = 1
    C_SUPP_OPT_CHUNK_MAP = 2
...
---------------------------------------------------------------------------------------------------
    ...
    RRPC_Protocol = {
        ...
        [C_RRPC_P2P_SETTING] = "P2P_SETTING",
...
---------------------------------------------------------------------------------------------------
    ...
    local f_pf_stun_ui16_checksum = ProtoField.uint16("ysstun.checksum", "checksum", base.DEC)
...
---- Reserved and Type
        ...
        stun_tree:add(f_pf_stun_ui16_checksum, buffer(offset, 2), buffer(offset, 2):uint())
        offset = offset + 2
...
---------------------------------------------------------------------------------------------------
    ...
    local f_pf_pene_ui16_checksum = ProtoField.uint16("yspene.checksum", "checksum", base.DEC)
    ...
    local function yspene_detector(buffer, pinfo, tree)
        ...
        elseif type == C_PENE_QUERYPEER_RSP then ---- QUERYPEER_RESP ---- validator
            if L ~= 30 + 2 then return false end ---- 检查此类型包的长度
        ...
        elseif type == C_PENE_REVERSING_REQ then ---- REVERSING_REQ ---- validator
            if L ~= 45 + 2 then return false end ---- 检查此类型包的长度
        elseif type == C_PENE_KEEPALIVE_REQ then ---- KEEPALIVE_REQ ---- validator
            if L ~= 1 + 2 then return false end ---- 检查此类型包的长度
        else
            return false
        end
        return true
...
---- Reserved and Type
        ...
        pene_tree:add(f_pf_pene_ui16_checksum, buffer(offset, 2), buffer(offset, 2):uint())
        offset = offset + 2
...
---------------------------------------------------------------------------------------------------
    ...
    local f_pf_supp_ui16_max_received_seqno = ProtoField.uint16("yssupp.max_received_seqno", "max_received_seqno", base.DEC)
    local f_pf_supp_ui32_accumulated_lost = ProtoField.uint32("yssupp.accumulated_lost", "accumulated_lost", base.DEC)
    ...
    local f_pf_supp_ui16_rppc = ProtoField.uint16("yssupp.rppc", "rppc", base.DEC)
    local f_pf_supp_ui32_bitrate = ProtoField.uint32("yssupp.bitrate", "bitrate", base.DEC)
    local f_pf_supp_ui64_ioffset = ProtoField.uint64("yssupp.ioffset", "iframe_offset", base.DEC)
    local f_pf_supp_ui64_ielapse = ProtoField.uint64("yssupp.ielapse", "iframe_elapse", base.DEC)
    ...
    local f_pf_supp_ui16_seqno = ProtoField.uint16("yssupp.seqno", "seqno", base.DEC)
    ...
    local f_pf_supp_ui16_checksum = ProtoField.uint16("yssupp.checksum", "checksum", base.DEC)
    local f_pf_supp_ui8_ack_opt = ProtoField.uint8("yssupp.ack_opt", "ack_opt", base.DEC)
    local f_pf_supp_ui8_opt_len = ProtoField.uint8("yssupp.opt_len", "opt_len", base.DEC)
    local f_pf_supp_ui16_nack_seq = ProtoField.uint16("yssupp.nack_seq", "nack_seq", base.DEC)
    local f_pf_supp_ui32_min_chunk_id = ProtoField.uint32("yssupp.min_chunk_id", "min_chunk_id", base.DEC)
    ...
    yssupp.fields = {
        ...
        f_pf_supp_ui16_max_received_seqno,
        f_pf_supp_ui32_accumulated_lost,
        ...
        f_pf_supp_ui16_checksum,
        f_pf_supp_ui8_ack_opt,
        f_pf_supp_ui8_opt_len,
        f_pf_supp_ui16_nack_seq,
        f_pf_supp_ui32_min_chunk_id
    }

    local function yssupp_detector(buffer, pinfo, tree)
        local L = buffer:len()
        if pinfo.dst_port ~= 9562 and pinfo.src_port ~= 9562 then
            return false
        end
        local type = bit:_and(buffer(0, 1):uint(), 0x0F)
        if type ~= C_SUPP_START_REQ and type ~= C_SUPP_START_RSP and type ~= C_SUPP_CHASE_REQ and type ~= C_SUPP_CHASE_RSP and type ~= C_SUPP_CHUNK_REQ and type ~= C_SUPP_CHUNK_RSP and type ~= C_SUPP_PIECE_DAT and type ~= C_SUPP_ACK and type ~= C_SUPP_FIN then
            return false
        end
        return true
    end
...
---- Reserved and Type
        ...
        if type == C_SUPP_START_REQ then
            ...
            offset = offset + 256
            local versiona = buffer(offset+0, 1):uint()
            local versionb = buffer(offset+1, 1):uint()
            local versionc = buffer(offset+2, 2):uint()
            local version_tree = supp_tree:add(yssupp, buffer(offset, 4), "sdk_version: "..versiona.."."..versionb.."."..versionc)
            offset = offset + 4
        ...
        if type == C_SUPP_PIECE_DAT then
            ...
            supp_tree:add(f_pf_supp_ui16_seqno, buffer(offset, 2))
            offset = offset + 2
            local ui32_chunk_id = buffer(offset, 4):uint()
        ...
        if type == C_SUPP_ACK then
            ...
            supp_tree:add(f_pf_supp_ui16_rppc, buffer(offset, 2))
            offset = offset + 2
            supp_tree:add(f_pf_supp_ui128_url_md5, buffer(offset, 16))
            offset = offset + 16
            local max_recv_seq = buffer(offset, 2):uint()
            supp_tree:add(f_pf_supp_ui16_max_received_seqno, buffer(offset, 2))
            offset = offset + 2
            local total_lost_pieces = buffer(offset, 4):uint()
            supp_tree:add(f_pf_supp_ui32_accumulated_lost, buffer(offset, 4))
            offset = offset + 4
            ...
            local ack_opt = buffer(offset, 1):uint()
            supp_tree:add(f_pf_supp_ui8_ack_opt, buffer(offset, 1))
            offset = offset + 1
            offset = offset + 7

            local next_ack_type = 0
            local loop = 0
            while true do
                if ack_opt == C_SUPP_OPT_END then
                    break
                end
                ...
                next_ack_type = buffer(offset, 1):uint()
                offset = offset + 1

                loop = loop + 1
                if ack_opt == C_SUPP_OPT_SEQ_NACK then
                    local len = buffer(offset, 1):uint()
                    supp_tree:add(f_pf_supp_ui8_opt_len, buffer(offset, 1))
                    offset = offset + 1
                    supp_tree:add(f_pf_supp_ui16_nack_seq, buffer(offset, 2))
                    offset = offset + 2
                    supp_tree:add(f_pf_supp_uix_bitmap, buffer(offset, len - 2))
                    offset = offset + len - 2
                elseif ack_opt == C_SUPP_OPT_CHUNK_MAP then
                    local len = buffer(offset, 1):uint()
                    supp_tree:add(f_pf_supp_ui8_opt_len, buffer(offset, 1))
                    offset = offset + 1
                    supp_tree:add(f_pf_supp_ui32_min_chunk_id, buffer(offset, 4))
                    offset = offset + 4
                    supp_tree:add(f_pf_supp_uix_bitmap, buffer(offset, len - 4))
                    offset = offset + len - 4
                else          
                    break
                end

                if next_ack_type == C_SUPP_OPT_END then 
                    break
                end

                ack_opt = next_ack_type 
            end
        end
        ...
        supp_tree:add(f_pf_supp_ui16_checksum, buffer(offset, 2), buffer(offset, 2):uint())
        offset = offset + 2
...
---------------------------------------------------------------------------------------------------
    ...
    local f_pf_puff_ui16_checksum = ProtoField.uint16("yspuff.checksum", "checksum", base.DEC)
...
---- Reserved and Type
        ...
        puff_tree:add(f_pf_puff_ui16_checksum, buffer(offset, 2), buffer(offset, 2):uint())
        offset = offset + 2
    ...
    local function yspuff_detector(buffer, pinfo, tree)
        local L = buffer:len()
        if pinfo.dst_port ~= 9560 and pinfo.src_port ~= 9562 then
            return false
        end
        local type = bit:_and(buffer(0, 1):uint(), 0x0F)
        if type ~= C_PUFF_SESSION_REQ and type ~= C_PUFF_SESSION_RSP and type ~= C_PUFF_SESSION_DAT and type ~= C_PUFF_SESSION_HIB and type ~= C_PUFF_SESSION_FIN then
            return false
        end
        return true
    end
...
---------------------------------------------------------------------------------------------------
    ...
    local f_pf_rrpc_checksum = ProtoField.uint16("ysrrpc.checksum", "checksum", base.DEC)
    local f_pf_rrpc_setting = ProtoField.uint8("ysrrpc.setting", "setting", base.DEC)
    ...
    local function ysrrpc_dissector(buffer, pinfo, tree)
        ...
        if rrpc_id == C_RRPC_P2P_SETTING then
            rrpc_tree:add(f_pf_rrpc_setting, buffer(offset, 1))
            offset = offset + 1
        end
        ...
        rrpc_tree:add(f_pf_rrpc_checksum, buffer(offset + url_len, 2), buffer(offset + url_len, 2):uint())
        offset = offset + url_len + 2
    ...
    local function ysrrpc_detector(buffer, pinfo, tree)
        ...
        if rrpc_id == C_RRPC_GENERAL_RSP then
            if L ~= 67 + 2 then return false end
        elseif rrpc_id == C_RRPC_JOINLF_REQ then
            if L ~= 408 + 2 then return false end
        elseif rrpc_id == C_RRPC_LEAVELF_REQ then
            if L ~= 82 + 2 then return false end
        elseif rrpc_id == C_RRPC_P2P_SETTING then 
            if L ~= 67 + 2 then return false end
        elseif rrpc_id == C_RRPC_INVESTIGATE_REQ then
            if L < 68 + 2 then return false end
...
---------------------------------------------------------------------------------------------------
    ...
    local f_pf_cccp_ui16_checksum = ProtoField.uint16("yscccp.checksum", "checksum", base.DEC)
    ...
    local function yscccp_dissector(buffer, pinfo, tree)
        ...
        local cccp_tree = tree:add(yscccp, buffer(0, L - 2), "Yunshang CCCP Protocol, "..CCCP_Protocol[type])
...
---------------------------------------------------------------------------------------------------
    ...
    local f_pf_dccp_ui16_checksum = ProtoField.uint16("ysdccp.checksum", "checksum", base.DEC)
    ...
    local function ysdccp_detector(buffer, pinfo, tree)
        ...
        if type == C_DCCP_TYPE_SYNC then
            if opt == 1 then return L > 31 + 2 else return L == 31 + 2 end
        elseif type == C_DCCP_TYPE_SYNACK then
            if opt == 1 then return L > 32 + 2 else return L == 32 + 2 end
        elseif type == C_DCCP_TYPE_DATA then
            if opt == 1 then return L > 15 + 2 else return L >= 15 + 2 end
        elseif type == C_DCCP_TYPE_ACK then
            if opt == 1 then return L > 16 + 2 else return L == 16 + 2 end
...
---- Handle Options
        ...
        if type == C_DCCP_TYPE_DATA then --DATA
            if yscccp_detector(buffer(offset):tvb(), pinfo, dccp_tree) then
                ...
                dccp_tree:add(f_pf_dccp_data, buffer(offset, L - offset - 2))
            end
            offset = L - 2   -- checksum
        end
        dccp_tree:add(f_pf_dccp_ui16_checksum, buffer(offset, 2), buffer(offset, 2):uint())
        offset = offset + 2
