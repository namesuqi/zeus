...
function install_hosts()
{
    ...
    if [ $file = "tsocks" ]; then
        ...
        sshpass -p $pass ssh -o StrictHostKeyChecking=no $user@$host "mv /etc/tsocks.conf /etc/tsocks.conf.bak.$now"
        # copy
        sshpass -p $pass scp -r tsocks.conf $user@$host:/etc/tsocks.conf

    elif [ $file = "hosts" ]
    then

        # delete previous files
        sshpass -p $pass ssh -o StrictHostKeyChecking=no $user@$host "mv /etc/hosts /etc/hosts.bak.$now"
        # copy
        sshpass -p $pass scp -r hosts $user@$host:/etc/hosts
