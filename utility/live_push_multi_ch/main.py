# coding=utf-8
"""
Main script to run multi-peer on Local machines
to verify how many channels Live-push can support
...
class RemoteNode(object):
    ...
    def stop_sdk(self):
        ...
        ssh.exec_command("pkill -9 {0}".format(SDK_FILE))
        ssh.close()
    ...
    def done(self):
        ...
        print command
        os.system(command)
        command = 'sshpass -p {0} ssh -o StrictHostKeyChecking=no {1}@${2} "pkill -9 liveclient_static"'.format(
            self.password, self.username, self.ip)
        print command
        os.system(command)
...
class RemoteDeployer(object):
    ...
    def deploy_folder(self, local_dir, remote_dir, kill_proc=None):
        ...
        if kill_proc is not None:
            ...
            ssh.exec_command("pkill -9 {0}".format(kill_proc))
